...
HANDLE_RESPONSES:
  ...
  if (state_mode == ENUM_VALUE)
    state_sequence = extract_state_codes(state_sequence, &state_count);
  // 从响应码中提取状态码
  else if (state_mode == RESPONSE_CODE){
    if (state_sequence) ck_free(state_sequence);
    state_sequence = (*extract_response_codes)(response_buf, response_buf_size, &state_count);
  }
...
//aflnet_go#
       ...
       "  -U core_id    - Starting number of CPU binding\n\n"
       "  -Z            - Use enum value as state id\n\n"
...
int main(int argc, char** argv) {
  ...
  SAYF(cCYA "aflnet_go " cBRI VERSION cRST " by <tangxiaofeng>\n");
...
//  param b is AFLGO's c
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Qz:b:N:A:D:W:w:e:P:KEq:s:RFZc:l:U:")) > 0)
//aflnet_go#
    switch (opt) {
...
//aflnet_go
      ...
      case 'b': { /* cut-off time for cooling schedule */
